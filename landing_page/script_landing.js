...
function handleRegister() {
  ...
  // Validações básicas
  if (!window._selectedRole) {
    alert('Por favor, selecione se você é Funcionário ou Gerente.');
    return;
  }
  
  if (!nameInput.value.trim()) {
    nameInput.focus();
    nameInput.style.borderColor = 'var(--vermelho)';
    nameInput.style.boxShadow = '0 0 0 3px rgba(255,94,126,.15)';
    setTimeout(() => {
  ...
  if (!emailInput.value.includes('@')) {
    emailInput.focus();
    emailInput.style.borderColor = 'var(--vermelho)';
    emailInput.style.boxShadow = '0 0 0 3px rgba(255,94,126,.15)';
    setTimeout(() => {
  ...
  if (pwInput.value.length < 8) {
    pwInput.focus();
    pwInput.style.borderColor = 'var(--vermelho)';
    pwInput.style.boxShadow = '0 0 0 3px rgba(255,94,126,.15)';
    setTimeout(() => {
  ...
  // Gerar código secreto para 2FA
  const secret = generateSecret();
  const userName = emailInput.value;
  const issuer = 'Cronos';
  
  // Guardar dados temporariamente
  window._registrationData = {
  ...
  // Gerar QR code
  generateQRCode(userName, secret, issuer);
  
  // Mostrar painel do QR code
  showPanel('qrcode');
...
function generateQRCode(userName, secret, issuer) {
  // Formatar URI OTP Auth
  ...
  if (typeof QRCode !== 'undefined') {
    // Criar container temporário para o QRCode.js
    ...
    // Esperar um pouco e então copiar para o canvas
    setTimeout(() => {
...
function copyManualCode() {
  ...
  navigator.clipboard.writeText(code).then(() => {
    const btn = event.target.closest('.qrcode-copy-btn');
    ...
    btn.innerHTML = `
      ...
      Copiado!
    `;
    btn.style.background = 'rgba(79,255,176,.15)';
...
function finishRegistration() {
  // Aqui você faria a chamada para o backend para criar o usuário
  // Por enquanto, apenas simula sucesso
  
  console.log('Registrando usuário:', window._registrationData);
  
  // Limpar dados temporários
  delete window._registrationData;
...
function verifyTwoFACode() {
  ...
  if (code.length !== 6 || !/^\d+$/.test(code)) {
    codeInput.style.borderColor = 'var(--vermelho)';
    codeInput.style.boxShadow = '0 0 0 3px rgba(255,94,126,.15)';
    setTimeout(() => {
  ...
  // Por enquanto, simula sucesso se o código for "427915"
  if (code === '427915') {
